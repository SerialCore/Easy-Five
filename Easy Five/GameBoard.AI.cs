...
namespace Easy_Five
{
    partial class GameBoard
    {
        ...
        {
            //建立估分表
            int[,] gradeMyMap = new int[N, N];
            int[,] gradeOppoMap = new int[N, N];
            for (int i = 0; i < N; i++)
            {
                for (int j = 0; j < N; j++)
                {
                    if (ChessBoard[i, j] == 0)
                    {
                        int sumMy = evaluate(i, j, color);
                        int sumOppo = evaluate(i, j, 3 - color);
            ...
            if (pMy != pOppo)
            {
                if (gradeOppoMap[pOppo.X, pOppo.Y] > gradeMyMap[pMy.X, pMy.Y] * 10)
                    return pOppo;
                else
                    return pMy;
            }
        ...
        {
            return NoCrossBorder(x, y) && ChessBoard[x, y] == color;
        }
        ...
        {
            return NoCrossBorder(x, y) && ChessBoard[x, y] == 0;
        }
